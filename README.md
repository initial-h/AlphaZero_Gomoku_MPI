...
* Randomness
  * **Dihedral Reflection or Rotation** : When use the network to output probabilities/value, it's better to do as paper said: The leaf node `$s_L$` is added to a queue for neural network evaluation, $(d_i(p),v)=f_{θ}(d_i(s_L))$, where $$d_i$$ is a dihedral reflection or rotation selected uniformly at random from `$i$` in `$[1..8]$`
